...
public class Character : ITriggedable
{
    ...
    /// 触发次数管理
    /// </summary>
    public SkillProcTimeMgr procTimeMgr;
    ...
    public Character(CharacterForRaid charaSource)
    {
        ...
        procTimeMgr = new SkillProcTimeMgr();
    }
    ...
    {
        Debug.Log("t>>OnSkillProc:" + skill.GetBaseData().name);
        //技能次数限制
        procTimeMgr.OnSkillProc(skill);
    }
    ...
    public void OnBeforeEndTurn(ActionContent sourceContent)
    {
        foreach (var skill in lstSkill)
        {
            skill.OnBeforeEndTurn(sourceContent);
        }
        foreach (var buff in lstBuffs)
        {
            buff.OnBeforeEndTurn(sourceContent);
        }
    }
}