    ...
    public class UIFightAction : MonoBehaviour
    {
        ...
        private void Refresh()
        {
            ...
            foreach (var skillData in _character.lstSkillData)
            {
                ...
                //非行动者,不能行动
                if (_character.camp == ECamp.Enemy || !_character.IsInReady() || GameMgr.Inst.HasActed(_character) || !_character.IsEnableAction)
                {
                    actionEnable = false;
                }
        ...
        private void RealSkillCast(SkillData skillData, Character target)
        {
            _character.OnActionSelected(skillData, target);
            UIMgr.Inst.uiFightActionRoot.SetVisible(false);
        }

        internal void OnSkillTargetClick(SkillData skillData, Character target)
        {
